    ...
    <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-white/5 rounded-lg p-3">
          <p className="text-xs text-white/40 mb-1">Daily burn</p>
          <p className="text-lg font-bold text-white">{fmt(velocity.daily_burn_rate)}</p>
        </div>
        <div className="bg-white/5 rounded-lg p-3">
          <p className="text-xs text-white/40 mb-1">Monthly burn</p>
          <p className="text-lg font-bold text-white">{fmt(velocity.monthly_burn_rate)}</p>
        </div>
        {velocity.savings_rate_pct != null && (
          <div className={`rounded-lg p-3 ${velocity.savings_rate_pct >= 20 ? 'bg-emerald-950/30 border border-emerald-500/30' : velocity.savings_rate_pct >= 0 ? 'bg-amber-950/30 border border-amber-500/30' : 'bg-red-950/30 border border-red-500/30'}`}>
            <p className="text-xs text-white/40 mb-1">Savings rate</p>
            <p className={`text-lg font-bold ${velocity.savings_rate_pct >= 20 ? 'text-emerald-400' : velocity.savings_rate_pct >= 0 ? 'text-amber-400' : 'text-red-400'}`}>
      ...
      {velocity.days_to_empty != null && velocity.days_to_empty < 30 && (
        <div className="flex items-center gap-2 text-sm text-amber-400 bg-amber-950/20 border border-amber-500/30 rounded-lg px-3 py-2">
      ...
      {velocity.monthly_surplus != null && (
        <p className="text-xs text-white/40">
          Monthly surplus after spending: <span className={velocity.monthly_surplus >= 0 ? 'text-emerald-400' : 'text-red-400'}>{fmt(velocity.monthly_surplus)}</span>
        </p>
      )}
    </div>
  )
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div className="bg-white/5 rounded-lg p-3">
          <p className="text-xs text-white/40 mb-1">Biggest spend day</p>
          <p className="font-semibold text-white">{patterns.most_expensive_day}</p>
          <p className="text-xs text-white/40">${patterns.avg_spend_by_day[patterns.most_expensive_day]?.toFixed(0) ?? '—'} avg</p>
        </div>
        <div className="bg-white/5 rounded-lg p-3">
          <p className="text-xs text-white/40 mb-1">Weekend vs Weekday</p>
          <p className="font-semibold text-white">{patterns.weekend_spend_pct.toFixed(0)}% weekends</p>
          <p className="text-xs text-white/40">{patterns.weekday_spend_pct.toFixed(0)}% weekdays</p>
        </div>
      </div>
      ...
      {patterns.month_end_spike && (
        <div className="flex items-center gap-2 text-xs text-amber-400 bg-amber-950/20 border border-amber-500/30 rounded-lg px-3 py-2">
        ...
        {visible.map((h, i) => (
          <div key={i} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
            <div className="min-w-0">
              <p className="text-sm font-medium text-white truncate">{h.merchant}</p>
              <p className="text-xs text-white/40">
                {h.visit_count} visits · {fmtDec(h.avg_per_visit)}/visit ·{' '}
                <span className="text-white/60">{h.habit_label}</span>
              </p>
            </div>
            <div className="text-right shrink-0 ml-3">
              <p className="text-sm font-semibold text-white">{fmt(h.monthly_total)}<span className="text-xs text-white/40">/mo</span></p>
              <p className="text-xs text-white/40">{fmt(h.annual_total)}/yr</p>
            </div>
          </div>
        ))}
      </div>
...
function ActionPlanCard({ actions }: { actions: ActionItem[] }) {
  ...
  return (
    <div className="rounded-xl border border-indigo-500/30 bg-indigo-950/10 p-4 space-y-4">
      <div className="flex items-start justify-between">
        <SectionHeader title="Your Action Plan" icon={Target} />
        {totalAnnual > 0 && (
          <div className="text-right">
            <p className="text-xs text-white/40">Total potential savings</p>
            <p className="text-lg font-bold text-emerald-400">{fmt(totalAnnual)}<span className="text-xs text-white/40">/yr</span></p>
          </div>
        )}
      </div>
      <div className="space-y-3">
        {actions.map((a, i) => (
          <div key={i} className={`rounded-lg border p-3 ${PRIORITY_COLOR[a.priority] || 'border-white/10'}`}>
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  ...
                  {a.annual_impact > 0 && (
                    <p className="text-sm font-bold text-emerald-400 shrink-0">{fmt(a.annual_impact)}<span className="text-xs text-white/40">/yr</span></p>
                  )}
                </div>
                <p className="text-xs text-white/50 mt-0.5">{a.detail}</p>
                <div className="flex items-center gap-3 mt-2">
                  <span className="text-xs text-white/30">{PRIORITY_LABEL[a.priority]}</span>
                  <span className="text-xs text-white/30 flex items-center gap-1"><Clock size={10} />{a.effort}</span>
                </div>
              </div>
            </div>
          </div>
...
function AffordableCard({ goals }: { goals: AffordableGoal[] }) {
  return (
    <div className="rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-4 space-y-3">
      ...
      <p className="text-xs text-white/40">If you act on the plan above, here's what becomes achievable:</p>
        ...
        {goals.map((g, i) => (
          <div key={i} className="flex items-center gap-3 bg-white/5 rounded-lg p-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">
                In <span className="text-emerald-400 font-medium">{g.months_to_reach.toFixed(0)} months</span>
                {' '}· {fmt(g.cost)} goal
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <div className="flex items-center gap-3">
                {c.pct_of_income != null && (
                  <span className="text-xs text-white/40">{c.pct_of_income.toFixed(1)}% of income</span>
                )}
                <span className="font-semibold text-white">{fmt(c.monthly_total)}<span className="text-xs text-white/40">/mo</span></span>
              </div>
            </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-base font-bold text-white">Personal Insights</h2>
        <span className="text-xs text-white/30 bg-white/5 px-2 py-0.5 rounded-full">deep analysis</span>
      </div>
